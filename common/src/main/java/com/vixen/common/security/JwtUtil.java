package com.vixen.common.security;
...
public class JwtUtil {
    ...
    public JwtUtil(@Value("${jwt.secret:${JWT_SECRET:}}") String secret, Environment environment) {
        this.secret = secret;
    ...
    private SecretKey getSigningKey() {
        String effectiveSecret = secret == null ? "" : secret.trim();

        if (effectiveSecret.isBlank()) {
            if (environment.acceptsProfiles(Profiles.of("prod"))) {
                throw new IllegalStateException("JWT secret must be configured for production");
            }
            effectiveSecret = "dev-only-secret-key-please-configure-in-production";
        }
